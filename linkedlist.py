...
class LinkedList(object):
    ...
    def print_list(self):
        ...
        # while current isn't 'None', print current and then point current to the 
        # next node in list.  The loop will stop at last item in list because the
    ...
    def get_node_by_index(self, idx):
        ...
        # If the index is 0 and the list isn't empty, we know this is the head
        if idx == 0:
            if self.head:
                return self.head
            else:
                print "There isn't an index 0, list is empty."
        ...
        # traverse until the counter reaches the item at index
        while counter <= idx:
            try:
                current = current.next
                ...
                print "..."
                print "Exception: List not long enough"
